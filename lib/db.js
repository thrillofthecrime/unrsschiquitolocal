...
export async function openDb(file) {
  ...
  const FEED_COLS = 'id, folder_id, url, title, site_url, last_error, last_fetched_at';

  const listFolders = () =>
    all('select id, name, position from folders order by position, name');

  const listFeeds = () =>
    all(`select ${FEED_COLS} from feeds
         order by case when title = '' then url else title end collate nocase`);

  const feedById = (id) => one(`select ${FEED_COLS} from feeds where id = ?`, id);
  ...
  const markFeedFetched = (id, { title, siteUrl, error = null }) => {
    ...
    run(`update feeds set last_fetched_at = ?, last_error = ?, title = ?, site_url = ?
         where id = ?`,
      now(), error, feed.title || title || '', feed.site_url || siteUrl || null, id);
    return feedById(id);
  };
