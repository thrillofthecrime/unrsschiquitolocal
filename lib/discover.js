...
async function probe(urls) {
  ...
  const found = (await Promise.all(urls.map(tryFeed))).filter(Boolean);
...
export async function discover(raw) {
  ...
  const found =
    (await probe(feedLinksFromHtml(page.body, page.url).slice(0, 5))) ||
    (await probe(siteSpecificCandidates(page.url, page.body))) ||
    (await probe(GUESS_PATHS.map((p) => origin + p)))?.slice(0, 4);
